; Plots daily CMORPH estimated precipitation
...
DEVICE, /INCHES, $
        ...
        /LANDSCAPE, $
        /COLOR, $
        FILENAME = figdir + 'cmorph_025deg_' + date + '.ps'
...
main_title = 'CMORPH daily precipitation (mm/day): '

;-----------------------------------------------------------------------
mmwSUM       = FLTARR(n_lon, n_lat)
mmwCNT       = FLTARR(n_lon, n_lat)
mmw_daily    = FLTARR(n_lon, n_lat)
cmorphSUM    = FLTARR(n_lon, n_lat)
cmorphCNT    = FLTARR(n_lon, n_lat)
cmorph_daily = FLTARR(n_lon, n_lat)
...
FOR ihr = 0, n_time-1 DO BEGIN
  ...
  FOR j = 0, n_lat-1 DO BEGIN
    IF (mmw_precip[i,j,ihr] NE missing) THEN BEGIN
      mmwSUM[i,j] = mmwSUM[i,j] + mmw_precip[i,j,ihr]
      mmwCNT[i,j] = mmwCNT[i,j] + 1
    ENDIF
    IF (cmorph_precip[i,j,ihr] NE missing) THEN BEGIN
      cmorphSUM[i,j] = cmorphSUM[i,j] + cmorph_precip[i,j,ihr]
...
FOR i = 0, n_lon-1 DO BEGIN
FOR j = 0, n_lat-1 DO BEGIN
  IF (mmwSUM[i,j] NE missing) THEN mmw_daily[i,j] = 24. * mmwSUM[i,j]
  IF (cmorphSUM[i,j] NE missing) THEN cmorph_daily[i,j] = 24. * cmorphSUM[i,j]
ENDFOR
ENDFOR
