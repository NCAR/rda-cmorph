...
! Global attributes
      ...
      status = nf90_put_att(ncid, NF90_GLOBAL, 'documentation', &
             'http://dss.ucar.edu/datasets/ds502.0')
      if (status /= NF90_NOERR) call handle_err(status,ncid,trim(prog))
      ...
      status = nf90_put_att(ncid, NF90_GLOBAL, 'acknowledgments', &
             "The data for this study are from the Research Data Archive (RDA) &
              ...
              &The original data are available from the RDA (http://dss.ucar.edu) &
...
! lon --------------------------------------------------------------------------
        ...
        status = nf90_put_att(ncid, lon_id, 'units', 'degrees_east')
        if (status /= NF90_NOERR) call handle_err(status,ncid,trim(prog))
...
! lat --------------------------------------------------------------------------
        ...
        status = nf90_put_att(ncid, lat_id, 'units', 'degrees_north')
        if (status /= NF90_NOERR) call handle_err(status,ncid,trim(prog))
...
!==============================================================================
      SUBROUTINE cmorph_def_var(var_nc, var_long_name, var_std_name, &
                                var_units)
!==============================================================================
!
